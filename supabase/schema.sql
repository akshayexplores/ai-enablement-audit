...
create index if not exists audit_responses_updated_idx on public.audit_responses (updated_at desc);
create index if not exists audit_responses_email_idx   on public.audit_responses (lower(email));
...
grant select, insert, update, delete on public.audit_responses to service_role;
...
declare
  ...
  v_rows int;
begin
  insert into public.audit_responses as r
