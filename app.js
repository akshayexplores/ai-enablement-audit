...
function animateCounts(root){
  ...
  els.forEach(el=>{
    const target=parseFloat(el.dataset.count)||0,dur=900,t0=performance.now();
    function step(t){
      const p=Math.min(1,(t-t0)/dur);
      el.textContent=pct(target*ease(p));
...
function render(keepScroll){
  ...
  if(key!==lastView){app.classList.remove("enter");void app.offsetWidth;app.classList.add("enter");animateCounts(app);window.scrollTo(0,0);const af=app.querySelector("[autofocus]");if(af)af.focus();}
